...
Bot in Python (solo standard library) che controlla i voli di continuita territoriale tramite l'API `travelOptions` e avvisa via Telegram quando un volo ha posti liberi.

## Cosa fa

- Esegue una singola interrogazione e termina.
- Supporta le tratte `PMO-PNL`, `PNL-PMO`, `TPS-PNL` e `PNL-TPS`.
- Se `PMO_PNL_RETURN_DATE` è vuota, effettua una ricerca di sola andata.
...
2. Personalizza `PMO_PNL_CITY_PAIR`, `PMO_PNL_DEPARTURE_DATE` (default: oggi) e, se vuoi controllare anche il rientro, `PMO_PNL_RETURN_DATE`.
3. Avvia il bot:
...
- `PMO_PNL_CITY_PAIR`: tratta, una tra `PMO-PNL`, `PNL-PMO`, `TPS-PNL`, `PNL-TPS`; default `PMO-PNL`.
...
- `RUN_ONCE`: mantenuta per compatibilità; il bot esegue sempre un solo controllo.
...
Il workflow `.github/workflows/pmo-pantelleria-monitor.yml` avvia una singola esecuzione ogni 5 minuti e usa la cache di GitHub Actions per mantenere lo stato tra le esecuzioni.
